...
Only one of `blockNumber` and `blockHash` should be set at a time; if both fields are set, the callee should raise a JSON-RPC error (the recommended error code is `-32602: Invalid params`).
...
- `eth_getStorageAt [ "0x<address>", { "blockHash": "0x<non-canonical-block-hash>", "blockNumber": "0x0" }` -> raise invalid-params error
