...
| `MAX_TRANSIENT_SLOTS` | 1 | The maximum number of transient slots allowed in a single transaction |
...
3. The gas cost for warm `SLOAD` (opcode `0x54`) is reduced from `GAS_WARM_ACCESS` (100) to `GAS_WARM_SLOAD`.
...
`TSTORE` currently has a fixed cost. However, writing to fresh slots requires memory allocation, which is more expensive than writing to an existing slot. Therefore, we may consider introducing charging more for the first slot allocation through a new parameter `GAS_TSTORE_ALLOCATE`. However, we would also need to introduce a mechanism to check for the first slot allocation versus subsequent allocations.
